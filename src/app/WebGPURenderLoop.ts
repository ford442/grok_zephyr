...
import {
  ...
  recordPassTimings,
} from '@/app/FrameProfilerEstimates.js';
...
export function recordTrailSamplesForCamera(
  ...
  const maxDistance = rt.camera.getViewMode() === 'moon' ? 240000 : rt.camera.getViewMode() === 'god' ? 140000 : 90000;
...
export function createWebGPURenderLoop(rt: AppRuntime): (timestamp: number) => void {
  const render = (timestamp: number): void => {
    if (!rt.isRunning || !rt.context || !rt.pipeline || !rt.earthVertexBuffer || !rt.earthIndexBuffer) {
      return;
    }
    ...
    if (rt.trailRenderer) {
      ...
      forward[1] /= fLen;
      forward[2] /= fLen;
      rt.trailRenderer.updateGeometry(rt.simTime, new Float32Array(cameraState.position), forward);
    }
    ...
    if (rt.camera.getViewMode() === 'skyline' && rt.context) {
      ...
      const sunPosSky = calculateSunPosition(rt.simTime);
      ...
      rt.skyline.updateUniform(
        ...
        nightFactor,
        rt.simTime,
        skylineEmissiveScale(rt.imageTuning.coreBoost),
      );
    ...
    if (rt.postProcessStack) {
      ...
      rt.pipeline.encodeCompositePass(encoder, screenView, canvasWidth, canvasHeight, motionBlurSourceView);
    }
    ...
    if (stats) {
      ...
      rt.ui.updateSimTime(rt.simTime);
    }
